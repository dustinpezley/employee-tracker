...
function showRoles() {
  const sql = `SELECT role.id, role.title, department.name AS department, CONCAT('$', FORMAT(role.salary, 0)) AS salary FROM role INNER JOIN department ON role.department_id = department.id`;
