...
function showEmployees() {
  const sql =`SELECT employee.id, employee.first_name, employee.last_name, role.title AS title, department.name AS department, CONCAT('$', FORMAT(role.salary, 0)) AS salary, CONCAT(m.first_name, ' ', m.last_name) AS manager FROM employee INNER JOIN employee m ON m.id = employee.manager_id RIGHT JOIN role ON employee.role_id = role.id RIGHT JOIN department ON role.department_id = department.id`;
